...
OSDCMSSpeedData::OSDCMSSpeedData(HttpClientWrapper *parent, OSDCmsConfig *cmsConfig): HttpClientWrapper(parent), cfgCms(cmsConfig)
{
...
OSDCMSSpeedData *OSDCMSSpeedData::getInstance(HttpClientWrapper *httpClient = nullptr, OSDCmsConfig *cmsConfig = nullptr)
{
...
BaseResponse<SpeedModel> OSDCMSSpeedData::toResponse(QByteArray raw)
{
    try {
        ...
        qDebug()<<Q_FUNC_INFO<<"resp"<<resp.getHttpCode()<<resp.getMessage()<<resp.getData()->getSpeed_OG()<<resp.getData()->getCourse_OG();
    ...
    ErrUnknown status;
    return BaseResponse<SpeedModel>(status.getCode(), status.getMessage(), nullptr);
}
...
BaseResponse<SpeedModel> OSDCMSSpeedData::errorResponse(QNetworkReply::NetworkError err)
{
    try {
        ...
        qDebug()<<Q_FUNC_INFO<<"caught error: "<<e.getMessage();
        return BaseResponse<SpeedModel>(e.getCode(), e.getMessage(), nullptr);
    }  catch (...) {
        qDebug()<<Q_FUNC_INFO<<"caught unkbnown error";
        ErrUnknown status;
        return BaseResponse<SpeedModel>(status.getCode(), status.getMessage(), nullptr);
